namespace NYH.CoreCardSystem
{
    ...
    public class CardSystem : Singleton<CardSystem>
    {
        ...
        protected override void Awake()
        {
            ...
            ActionSystem.AttachPerformer<CostPlusGA>(action => Perform(action));                  //코스트 증가 액션
            ActionSystem.AttachPerformer<ZeroCostHandGA>(action => Perform(action));              //손패 비용 0 액션
        ...
        public IEnumerator Perform(GameAction action)
        {
            ...
            {
                // 1. 인스펙터에서 설정한 cardAmount만큼의 카드 코스트를 costAmount로 변경
                int modifiedCount = 0;
                foreach (var card in hand)
                {
                    // cardAmount가 0이면 손패 전체, 그 외엔 지정된 수만큼 변경
                ...
                // 2. UI 갱신: 현재 씬의 모든 CardView를 찾아 텍스트를 업데이트
                CardView[] allViews = Object.FindObjectsByType<CardView>(FindObjectsSortMode.None);
                foreach (var view in allViews)
                {
